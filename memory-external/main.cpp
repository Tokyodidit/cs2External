...
int main() {
	SetConsoleTitle("External Esp made By Tokyodidit");

	hack::process = std::make_shared<pProcess>();

	std::cout << "[cs2] Waiting..." << std::endl;
	Beep(500, 100);

	while (!hack::process->AttachProcess("cs2.exe"))
		std::this_thread::sleep_for(std::chrono::seconds(1));

	std::cout << "Attached to cs2.exe" << std::endl;
	Beep(500, 100);
	...
	std::cout << "[Tranquillity] Creating overlay..." << std::endl;
	...
	HWND hWnd = CreateWindowExA(WS_EX_TRANSPARENT | WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_TOOLWINDOW, " ", "Tranquillity", WS_POPUP,
